...
```{r, include=FALSE}
knitr::opts_chunk$set(echo = TRUE,warning=F,message=F,eval=T, fig.height=4,cache=T)
```
...
```{r,eval=F}
glm_1 <- glm(children ~ age,data=dat1, 
                    family = binomial())
```

```{r,eval=F,echo=F}
save(glm_1,file="../data/glm_1.RData")
```

```{r,echo=FALSE}
load("../data/glm_1.RData")
```
...
`r faraway::ilogit(sum_glm1$coefficients[2,1])`.
...
library(ggplot2)
dfex <- data.frame(age=dat1$age,fit_prob)
ggplot(aes(x=age, y=fit_prob),data=dfex)  +
  geom_line(aes(x=age, y=fit_prob),data=dfex)
```
...
```{r,eval=F,echo=F}
save(glm_2,file="../data/glm_2.RData")
```

```{r,echo=F}
library(pscl)
load("../data/glm_2.RData")
```
...
```{r,eval=F,echo=F}
save(probitmod,file="../data/probitmod.RData")
```

```{r,echo=F}
load("../data/probitmod.RData")
```
...
```{r,eval=F,echo=F}
save(modp,file="../data/modp.RData")
```

```{r,echo=F}
load("../data/modp.RData")
```
...
<!--
Aufgabe logistische Regression

https://tutorials.iq.harvard.edu/R/Rstatistics/Rstatistics.html#exercise_2:_logistic_regression
-->
...
- [**Kategoriale Daten:**](http://homepage.univie.ac.at/herbert.nagel/KategorialeDaten.pdf) - [**Durchführung logistische Regression in R**](https://www.r-bloggers.com/how-to-perform-a-logistic-regression-in-r/)


<!--
Augabe logistische Regression
https://tutorials.iq.harvard.edu/R/Rstatistics/Rstatistics.html#exercise_2:_logistic_regression
-->
